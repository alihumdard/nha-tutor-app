...
<body>
    ...
    <main>
        ...
        <div class="plan-grid">
            ...
            $isCurrentPlan = $isSubscribed && auth()->user()->subscription('default')->hasPrice($plan['stripe_price_id']
            ?? null);
            @endphp
            <div class="plan {{ $isCurrentPlan ? 'current-plan' : '' }}">
                ...
                @elseif ($isSubscribed)
                @if (!empty($plan['stripe_price_id']))
                <a href="{{ route('subscribe.swap', ['priceId' => $plan['stripe_price_id']]) }}" class="btn-swap">Swap
                    Plan</a>
                @else
                <span class="btn-disabled" title="Plan not available">Not Available</span>
                @endif
                @else
                @if (!empty($plan['stripe_price_id']))
                <a href="{{ route('subscribe', ['priceId' => $plan['stripe_price_id']]) }}"
